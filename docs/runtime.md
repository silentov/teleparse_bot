...
2. берёт redis-lock на пару `(chat_id, user_id)`;
3. загружает FSM-контекст;
4. если текст начинается с `/`, сначала ищет command-handler;
5. иначе ищет handler текущего состояния;
6. если handler не найден — использует fallback `default`.
...
2. берётся тот же redis-lock на пользователя;
...
5. пользователь выбирает лимит или вводит число вручную;

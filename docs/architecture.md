...
- короткие критические секции redis-lock для чтения/записи FSM;
- запуск долгих операций (Telegram history + LLM) вне lock для снижения риска истечения TTL;
- защиту от параллельной обработки через processing-флаг и controlled-response при конфликте.
